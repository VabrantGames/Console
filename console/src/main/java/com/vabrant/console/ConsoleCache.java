...
public class ConsoleCache {
	...
	public boolean hasMethod (ClassReference<?> classReference, String methodName, Class<?>... args) {
		...
		for (MethodInfo info : methods) {
			...
			if (ref.getName().equals(methodName)
				&& ConsoleUtils.areArgsEqual(ref.getArgs(), ConsoleUtils.defaultIfNull(args, ConsoleUtils.EMPTY_ARGUMENT_TYPES)))
				return true;
		}
		return false;
	}
	...
	public void addReference (Object object, String referenceID) {
		...
		if (referenceID == null || referenceID.isEmpty()) {
			if (object.getClass().isAnnotationPresent(ConsoleObject.class)) {
				ConsoleObject o = ClassReflection.getAnnotation(object.getClass(), ConsoleObject.class)
					.getAnnotation(ConsoleObject.class);
		...
		if (instanceReference == null) {
			if (object.getClass().isAnnotationPresent(ConsoleObject.class)) {
				...
				if (hasReference(objectID)) {
					ConsoleObject c = ClassReflection.getAnnotation(object.getClass(), ConsoleObject.class)
						.getAnnotation(ConsoleObject.class);
	...
	/** Saves an instance of a method for a class to be shared by other instances of the class. <br>
	 * <br>
	 */
	public static class MethodLookup {
		...
		MethodReference getReferenceMethod (Class<?> declaringClass, String name, Class<?>... argTypes) {
			...
			while (it.hasNext()) {
				...
				if (ref.getName().equals(name) && ConsoleUtils.areArgsEqual(ref.getArgs(),
					ConsoleUtils.defaultIfNull(argTypes, ConsoleUtils.EMPTY_ARGUMENT_TYPES))) {
					return ref;
				}
			}
			return null;
		...
		MethodReference addReferenceMethod (Method method) {
			...
			if (classMethodReferences == null) {
				...
				stringBuilder.clear();
				stringBuilder.append(ADDED_TAG).append(" [Reference]:Class").append(NAME_TAG)
					.append(method.getDeclaringClass().getSimpleName()).append(FULL_TAG)
			...
			if (reference == null) {
				...
				stringBuilder.clear();
				stringBuilder.append(ADDED_TAG).append(" [Reference]:Method ").append(NAME_TAG).append(method.getName())
					.append(" [Args]:").append(argsToString(method.getParameterTypes())).append(" [DeclaringClass]:")
					.append(method.getDeclaringClass().getSimpleName()).append(FULL_TAG)
					.append(method.getDeclaringClass().getCanonicalName());
				logger.info(stringBuilder.toString());
			}
