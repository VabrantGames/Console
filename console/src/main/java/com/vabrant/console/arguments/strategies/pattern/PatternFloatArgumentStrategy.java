...
public class PatternFloatArgumentStrategy implements Argument.ArgumentStrategy<PatternStrategyInput> {
	...
	public PatternFloatArgumentStrategy () {
		pattern = PatternBuilder.getInstance()
			// e.g 100f
			.addRule(DIGIT | ONE_OR_MORE).addRule(CUSTOM, "fF").or()

			// e.g .100f
			.addRule(CUSTOM, ".").addRule(DIGIT | ONE_OR_MORE).addRule(CUSTOM, "fF").or()

			// e.g 100.0f
			.addRule(DIGIT | ONE_OR_MORE).addRule(CUSTOM, ".").addRule(DIGIT | ZERO_OR_MORE).addRule(CUSTOM, "fF").build();
	}
