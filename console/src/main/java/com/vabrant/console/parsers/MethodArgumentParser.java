	...
	public MethodArgumentInfo parse (ConsoleCacheAndStringInput input) throws RuntimeException {
		...
		if (text.charAt(0) == '.') {
			...
			if (text.contains(".")) {
				...
				if (!cache.hasMethodWithName(info.getClassReference(), info.getMethodName()))
					throw new RuntimeException("[MethodNotFound] : [Name]:" + info.getMethodName() + " [Reference]:" + referenceName);
		...
		if (info.getClassReference() == null && info.getMethods() == null)
			throw new RuntimeException("[ParsingError] : [Input]:" + text);
